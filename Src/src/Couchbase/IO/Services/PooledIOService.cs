...
namespace Couchbase.IO.Services
{
    ...
    public class PooledIOService : IIOService
    {
        ...
        private void Authenticate(IConnection connection)
        {
            if (!connection.IsAuthenticated)
            {
                if (_saslMechanism != null)
                {
                    ...
                    if (result)
                    {
                        Log.LogDebug("Authenticated {0} using {1} - {2}.", _saslMechanism.Username,
                                    _saslMechanism.GetType(), _identity);
                        connection.IsAuthenticated = true;
                    }
                    else
                    {
                        Log.LogDebug("Could not authenticate {0} using {1} - {2}.", _saslMechanism.Username,
                                    _saslMechanism.GetType(), _identity);
        ...
        public void Dispose()
        {
            Log.LogDebug("Disposing PooledIOService for {0} - {1}", EndPoint, _identity);
            Dispose(true);
        }
        ...
        ~PooledIOService()
        {
            Log.LogDebug("Finalizing PooledIOService for {0} - {1}", EndPoint, _identity);
            Dispose(false);
        }
#endif
    }
