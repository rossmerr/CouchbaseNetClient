...
namespace Couchbase.Core
{
    ...
    internal class Server : IServer
    {
        ...
        public Server(IIOService ioService, INodeAdapter nodeAdapter, ClientConfiguration clientConfiguration,
            IBucketConfig bucketConfig, ITypeTranscoder transcoder, ILogger logger) :
            this(ioService,
                    new ViewClient(new HttpClient(), new JsonDataMapper(clientConfiguration), bucketConfig, clientConfiguration, logger),
                    new QueryClient(new HttpClient(), new JsonDataMapper(clientConfiguration), bucketConfig, clientConfiguration, logger),
                    new SearchClient(bucketConfig, clientConfiguration, new SearchDataMapper(), logger),
                    nodeAdapter, clientConfiguration, transcoder, bucketConfig, logger)
        {
        }

        public Server(IIOService ioService, INodeAdapter nodeAdapter, ClientConfiguration clientConfiguration,
            IBucketConfig bucketConfig, ITypeTranscoder transcoder, ConcurrentDictionary<string, QueryPlan> queryCache, ILogger logger) :
                this(ioService,
                    new ViewClient(new HttpClient(), new JsonDataMapper(clientConfiguration), bucketConfig, clientConfiguration, logger),
                    new QueryClient(new HttpClient(), new JsonDataMapper(clientConfiguration), bucketConfig, clientConfiguration, queryCache, logger),
                    new SearchClient(bucketConfig, clientConfiguration, new SearchDataMapper(), logger),
                    nodeAdapter, clientConfiguration, transcoder, bucketConfig, logger)
        {
        }
        ...
        public Server(IIOService ioService, IViewClient viewClient, IQueryClient queryClient, ISearchClient searchClient,
            ...
            ClientConfiguration clientConfiguration, ITypeTranscoder transcoder, IBucketConfig bucketConfig, ILogger logger)
        {
            Log = logger;
            if (ioService != null)
            {
            ...
            if (IsDataNode || IsQueryNode)
            {
                ...
                Log.LogInformation("Initialization {0} for node {1}", _isDown ? "failed" : "succeeded", EndPoint);

                //timer and node status
                //_heartBeatTimer = new Timer(_heartBeatTimer_Elapsed);
                AutoResetEvent autoEvent = new AutoResetEvent(false);
                _heartBeatTimer = new Timer(_heartBeatTimer_Elapsed, autoEvent, Timeout.Infinite, Timeout.Infinite);
                if (_isDown)
                {
                    StartHeartbeatTimer();
                }
        ...
        {
            Log.LogInformation("Checking if node {0} is down: {1}", EndPoint, _isDown);
        ...
        void CheckQueryNode()
        {
            try
            {
                ...
                if (result.Success)
                {
                    Log.LogInformation("Successfully connected and marking query node {0} as up.", EndPoint);
                    _isDown = false;
                }
                else
                {
                    Log.LogInformation("The query node {0} is still down: {1}", EndPoint, result.Status);
                }
            ...
            catch (Exception e)
            {
                ...
                Log.LogInformation("The query node {0} is still down: {1}", EndPoint, e.Message);
        ...
        void CheckDataNode()
        {
            ...
            try
            {
                ...
                if (result.Success)
                {
                    Log.LogInformation("Successfully connected and marking data node {0} as up.", EndPoint);
                    _isDown = false;
                }
                else
                {
                    Log.LogInformation("The data node {0} is still down: {1}", EndPoint, result.Status);
                }
            }
            ...
            catch (Exception e)
            {
                ...
                Log.LogInformation("The data node {0} is still down: {1}", EndPoint, e.Message);
                //the node is down or unreachable
                _isDown = true;
                Log.LogDebug(e.Message);
            }
        ...
        public void CheckOnline(bool isDead)
        {
            if (isDead && IsDataNode)
            {
                lock (_syncObj)
                {
                    ...
                    Log.LogInformation("Checking if node {0} should be down - last: {1}, current: {2}, count: {3}",
                    ...
                    if (_ioErrorCount > _clientConfiguration.IOErrorThreshold)
                    {
                        if(last < current)
                        {
                            Log.LogInformation("Marking node {0} as down - last: {1}, current: {2}, count: {3}",
                               EndPoint, last.TimeOfDay, current.TimeOfDay, _ioErrorCount);
        ...
        public IOperationResult Send(IOperation operation)
        {
            ...
            if (Log.IsEnabled(LogLevel.Debug) && _timingEnabled)
            {
            ...
            else
            {
                try
                {
                    Log.LogDebug("Sending {0} with key {1} using server {2}", operation.GetType().Name, operation.Key, EndPoint);
        ...
        public IOperationResult<T> Send<T>(IOperation<T> operation)
        {
            ...
            else
            {
                try
                {
                    Log.LogInformation("Sending {0} with key {1} using server {2}", operation.GetType().Name, operation.Key, EndPoint);
                    result = _ioService.Execute(operation);
                }
                catch (Exception e)
                {
        ...
        public void Dispose()
        {
            Log.LogInformation("Disposing Server for {0}", EndPoint);
            Dispose(true);
        }
        ...
        ~Server()
        {
            Log.LogInformation("Finalizing Server for {0}", EndPoint);
            Dispose(false);
        }
#endif
    }
