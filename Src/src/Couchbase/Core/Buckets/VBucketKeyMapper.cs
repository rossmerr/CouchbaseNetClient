...
namespace Couchbase.Core.Buckets
{
    ...
    internal class VBucketKeyMapper : IKeyMapper
    {
        ...
        public IMappedNode MapKey(string key)
        {
            ...
            Log.LogTrace("Using index {0} for key {1} - rev{2}", index, key, Rev);
        ...
        Dictionary<int, IVBucket> CreateVBucketMap()
        {
            ...
            Log.LogInformation("Creating VBuckets {0} and FMaps {1} for Rev#{2}", vBucketMap.Length,
            ...
            for (var i = 0; i < vBucketMap.Length; i++)
            {
                ...
                vBuckets.Add(i, new VBucket(_servers, i, primary, replicas, Rev, _vBucketServerMap, _bucketName, Log));
            }
            return vBuckets;
        }
        ...
        Dictionary<int, IVBucket> CreateVBucketMapForwards()
        {
            ...
            {
                Log.LogInformation("Creating VBucketMapForwards {0}", vBucketMapForward.Length);
                ...
                for (var i = 0; i < vBucketMapForward.Length; i++)
                {
                    ...
                    vBucketMapForwards.Add(i, new VBucket(_servers, i, primary, replicas, Rev, _vBucketServerMap, _bucketName, Log));
                }
            }
