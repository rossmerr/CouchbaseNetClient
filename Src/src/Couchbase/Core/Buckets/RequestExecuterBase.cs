    ...
    internal abstract class RequestExecuterBase : IRequestExecuter
    {
        ...
        public void LogFailure(IOperation operation, IOperationResult operationResult)
        {
            ...
            if (vBucket != null)
            {
                ...
                Log.LogDebug(msg1, operation.Key, operation.Attempts, vBucket.Index, vBucket.Rev, operation.Opaque, operationResult.Message);
            }
            else
            {
                const string msg1 = "Operation for key {0} failed after {1} retries and opaque{2}. Reason: {3}";
                Log.LogDebug(msg1, operation.Key, operation.Attempts, operation.Opaque, operationResult.Message);
            }
        }
        ...
        public void UpdateConfig()
        {
            ...
            {
                Log.LogInformation("Updating config on {0} using rev#{1}", node.EndPoint, node.Revision);
