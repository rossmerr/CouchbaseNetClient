...
namespace Couchbase.Configuration
{
    ...
    internal abstract class ConfigContextBase : IConfigInfo
    {
        ...
        {
            Log.LogTrace("Getting KeyMapper for rev#{0} on thread {1}", BucketConfig.Rev, Thread.CurrentThread.ManagedThreadId);
            return KeyMapper;
        }
        ...
        public void Dispose()
        {
            Log.LogDebug("Disposing ConfigContext");
            Dispose(true);
        }
        ...
        ~ConfigContextBase()
        {
            Log.LogDebug("Finalizing ConfigContext for Rev#{0}", BucketConfig.Rev);
            Dispose(false);
        }
#endif
