...
namespace Couchbase
{
    ...
    public sealed class CouchbaseBucket : IBucket, IConfigObserver, IRefCountable, IQueryCacheInvalidator, ISubdocInvoker
    {
        ...
        {
            Log.LogInformation("Config updated old/new: {0}, {1}",
        ...
        public IOperationResult<string> Append(string key, string value)
        {
            ...
            {
                Log.LogInformation("Requires retry {0}", key);
            }
        ...
        {
            CheckDisposed();
            return new ViewQuery(Name, designDoc, view, Log)
            {
        ...
        {
            CheckDisposed();
            return new ViewQuery(Name, designdoc, viewname, Log)
            {
                UseSsl = _configInfo.SslConfigured
            }
        ...
        public void Dispose()
        {
            Log.LogDebug("Attempting dispose on thread {0}", Thread.CurrentThread.ManagedThreadId);
        ...
        int IRefCountable.AddRef()
        {
            lock (RefCounts)
            {
                if (!_disposed)
                {
                    ...
                    Log.LogDebug("Creating bucket refCount# {0}", refCount.Count);
        ...
        int IRefCountable.Release()
        {
            lock (RefCounts)
            {
                ...
                if (refCount.Count > 0)
                {
                    Log.LogDebug("Current bucket refCount# {0}", refCount.Count);
                    Interlocked.Decrement(ref refCount.Count);
                    if (refCount.Count != 0) return refCount.Count;
                    Log.LogDebug("Removing bucket refCount# {0}", refCount.Count);
...
;
        ...
        public bool IsSecure
        {
            get
            {
                ...
                {
                    Log.LogInformation("Default to IsSecure false because of {0}", e);
                    return false;
                }
            }
        }
        ...
        bool CheckForConfigUpdates<T>(IOperationResult<T> operationResult, IOperation operation)
        {
            ...
            if (operationResult.Status == ResponseStatus.VBucketBelongsToAnotherServer)
            {
                try
                {
                    ...
                    {
                        Log.LogInformation("New config found {0}|{1}: {2}", bucketConfig.Rev, _configInfo.BucketConfig.Rev, JsonConvert.SerializeObject(bucketConfig));
        ...
        void Dispose(bool disposing)
        {
            if (!_disposed)
            {
                Log.LogDebug("Disposing on thread {0}", Thread.CurrentThread.ManagedThreadId);
