...
namespace Couchbase.IO.Services
{
    ...
    public class MultiplexingIOService : IIOService
    {
        ...
        public IOperationResult<T> Execute<T>(IOperation<T> operation)
        {
            ...
            {
                Log.LogDebug("Endpoint: {0} - {1} {2}", EndPoint, _identity, e);
        ...
        private void Authenticate(IConnection connection)
        {
            if (!connection.IsAuthenticated)
            {
                if (SaslMechanism != null)
                {
                    ...
                    if (result)
                    {
                        Log.LogDebug("Authenticated {0} using {1} - {2} [{3}].", SaslMechanism.Username, SaslMechanism.GetType(),
                                    _identity, EndPoint);
                        connection.IsAuthenticated = true;
                    }
                    else
                    {
                        Log.LogDebug("Could not authenticate {0} using {1} - {2} [{3}].", SaslMechanism.Username,
        ...
        public void Dispose()
        {
            Log.LogDebug("Disposing IOService for {0} - {1}", EndPoint, _identity);
            lock (_syncObj)
            {
                Dispose(true);
            }
        ...
        {
            Log.LogDebug("Finalizing IOService for {0} - {1}", EndPoint, _identity);
            Dispose(false);
        }
#endif
    }
