...
namespace Couchbase.IO
{
    public class Connection : ConnectionBase
    {
        ...
        internal Connection(IConnectionPool connectionPool, Socket socket, IByteConverter converter, BufferAllocator allocator, ILogger logger)
        ...
        public override void SendAsync(byte[] buffer, Func<SocketAsyncState, Task> callback)
        {
            ...
            try
            {
                ...
                Log.LogDebug("Sending {0} with {1} on server {2}", state.Opaque, Identity, EndPoint);
        ...
        public override byte[] Send(byte[] buffer)
        {
            ...
            Log.LogDebug("Sending opaque{0} on {1}", state.Opaque, Identity);
            _eventArgs.UserToken = state;
            ...
            {
                Log.LogDebug("Connection {0} has failed with {1}", Identity, state.Exception);
                IsDead = true;
                throw state.Exception;
            }

            Log.LogDebug("Complete opaque{0} on {1}", state.Opaque, Identity);
        ...
        private void Send(Socket socket, SocketAsyncEventArgs e)
        {
            ...
            else
            {
                ...
                Log.LogDebug("Error: {0} - {1}", Identity, state.Exception);
        ...
        public void Receive(Socket socket, SocketAsyncEventArgs e)
        {
            ...
            {
                var state = (SocketAsyncState)e.UserToken;
                Log.LogDebug("Receive {0} bytes for opaque{1} with {2} on server {3} offset{4}", e.BytesTransferred, state.Opaque, Identity, EndPoint, state.SendOffset);
                if (e.SocketError == SocketError.Success)
                {
                    //socket was closed on recieving side
                    if (e.BytesTransferred == 0)
                    {
                        Log.LogDebug("Connection {0} has failed in receive with {1} bytes.", Identity, e.BytesTransferred);
                        IsDead = true;
                        if (state.Completed == null)
                        {
                            if (!Disposed)
                    ...
                    else
                    {
                        ...
                        if (state.Completed == null)
                        {
                            Log.LogDebug("Complete with set {0} with {1} on server {2}", state.Opaque, Identity, EndPoint);
                            _requestCompleted.Set();
                        }
                        else
                        {
                            Log.LogDebug("Complete {0} with {1} on server {2}", state.Opaque, Identity, EndPoint);
                            ConnectionPool.Release(this);
                            state.Completed(state);
                        }
                    }
