...
namespace Couchbase.N1QL
{
    ...
    internal class QueryClient : IQueryClient
    {
        ...
        public QueryClient(HttpClient httpClient, IDataMapper dataMapper, IBucketConfig bucketConfig, ClientConfiguration clientConfig, ILogger logger)
            : this(httpClient,dataMapper, bucketConfig, clientConfig, new ConcurrentDictionary<string, QueryPlan>(), logger)
        {
        }

        public QueryClient(HttpClient httpClient, IDataMapper dataMapper, IBucketConfig bucketConfig, ClientConfiguration clientConfig, ConcurrentDictionary<string, QueryPlan> queryCache, ILogger logger)
        {
            Log = logger;
        ...
        void PrepareStatementIfNotAdHoc(IQueryRequest originalRequest)
        {
            ...
            else
            {
                ...
                if (!result.Success)
                {
                    Log.LogWarning("Failure to prepare plan for query {0} (it will be reattempted next time it is issued): {1}",
        ...
        async Task PrepareStatementIfNotAdHocAsync(IQueryRequest originalRequest)
        {
            ...
            else
            {
                ...
                if (!result.Success)
                {
                    Log.LogWarning("Failure to prepare async plan for query {0} (it will be reattempted next time it is issued): {1}",
                        originalStatement, result.GetErrorsAsString());
        ...
        private async Task<IQueryResult<T>> ExecuteQueryAsync<T>(IQueryRequest queryRequest)
        {
            ...
            using (var content = new StringContent(queryRequest.GetFormValuesAsJson(), System.Text.Encoding.UTF8, "application/json")) {
                try
                {
                    Log.LogTrace("Sending query cid{0}: {1}", queryRequest.CurrentContextId, baseUri);
                    ...
                    using (var response = await request.Content.ReadAsStreamAsync().ContinueOnAnyContext())
                    {
                        ...
                        Log.LogTrace("Received query cid{0}: {1}", queryResult.ClientContextId, queryResult.ToString());
                    }
                    baseUri.ClearFailed();
                }
                catch (HttpRequestException e)
                {
                    Log.LogInformation("Failed query cid{0}: {1}", queryRequest.CurrentContextId, baseUri);
