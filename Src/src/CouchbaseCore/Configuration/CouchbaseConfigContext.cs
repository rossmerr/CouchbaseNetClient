    ...
    internal sealed class CouchbaseConfigContext : ConfigContextBase
    {
        ...
        public override void LoadConfig(IBucketConfig bucketConfig, bool force = false)
        {
            try
            {
                ...
                {
                    Log.LogInformation("o1-Creating the Servers {0} list using rev#{1}", nodes.Count, bucketConfig.Rev);
                    ...
                    foreach (var adapter in nodes)
                    {
                        ...
                        try
                        {
                            Log.LogInformation("Creating node {0} for rev#{1}", endpoint, bucketConfig.Rev);
                            IServer server;
                        ...
                        {
                            Log.LogError("Could not add server {0} for rev#{1}. Exception: {2}", endpoint, bucketConfig.Rev, e);
                        }
                    }
                    ...
                    Log.LogInformation("Creating the KeyMapper list using rev#{0}", bucketConfig.Rev);
                    ...
                    if (old != null)
                    {
                        ...
                        {
                            Log.LogInformation("Disposing node {0} from rev#{1}", server.EndPoint, server.Revision);
                            server.Dispose();
                        }
                        old.Clear();
                    }
                ...
                else
                {
                    if (BucketConfig == null || !BucketConfig.IsVBucketServerMapEqual(bucketConfig) || force)
                    {
                        ...
                        var vBucketKeyMapper = new VBucketKeyMapper(Servers, bucketConfig.VBucketServerMap,
                            bucketConfig.Rev, bucketConfig.Name, Log);
                        Interlocked.Exchange(ref KeyMapper, vBucketKeyMapper);
                        Interlocked.Exchange(ref _bucketConfig, bucketConfig);
                    }
                }
        ...
        public void LoadConfig(IIOService ioService)
        {
            ...
            {
                Lock.EnterWriteLock();
                Log.LogInformation("o2-Creating the Servers list using rev#{0}", BucketConfig.Rev);
