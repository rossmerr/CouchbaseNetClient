        ...
        public IBucket CreateBucket(string bucketName, string password)
        {
            ...
            lock (_syncObject)
            {
                ...
                if (_buckets.ContainsKey(bucketName))
                {
                    ...
                    {
                        Log.LogDebug("Bootstraping was already done, returning existing bucket {0}", bucketName);
                        return existingBucket; // This is the only short circuit. All other cases fall through to bootstrapping.
                    }
                    else
                    {
                        Log.LogDebug("Bucket dictionary contained disposed bucket. Bootstrapping {0}.", bucketName);
                ...
                foreach (var provider in _configProviders)
                {
                    try
                    {
                        Log.LogDebug("Trying to bootstrap with {0}.", provider);
                        ...
                        {
                            Log.LogDebug("Using existing bootstrap {0}.", provider);
                        ...
                        {
                            Log.LogDebug("Successfully bootstrapped using {0}.", provider);
                            _clientConfig.UpdateBootstrapList(config.BucketConfig);
                            configObserver.NotifyConfigChanged(config);
                            success = true;
                            break;
