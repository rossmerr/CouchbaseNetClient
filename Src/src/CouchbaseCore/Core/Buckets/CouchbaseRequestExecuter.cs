    ...
    internal class CouchbaseRequestExecuter : RequestExecuterBase
    {
        ...
        public bool CheckForConfigUpdates(IOperation operation)
        {
            ...
            try
            {
                ...
                {
                    Log.LogInformation("New config found {0}|{1}", bucketConfig.Rev, ConfigInfo.BucketConfig.Rev);
                    Log.LogDebug(JsonConvert.SerializeObject(bucketConfig));
        ...
        async Task<IViewResult<T>> RetryViewEveryAsync<T>(Func<IViewQueryable, IConfigInfo, Task<IViewResult<T>>> execute,
            ...
            CancellationToken cancellationToken)
        {
            while (true)
            {
                ...
                Log.LogDebug("trying again: {0}", query.RetryAttempts);
        ...
        async Task<IQueryResult<T>> RetryQueryEveryAsync<T>(Func<IQueryRequest, IConfigInfo, Task<IQueryResult<T>>> execute,
            IQueryRequest query,
            IConfigInfo configInfo,
            CancellationToken cancellationToken)
        {
            ...
            while (true)
            {
                ...
                Log.LogDebug("trying query again: {0}", attempts);
        ...
        {
            if (Log.IsEnabled(LogLevel.Debug) && TimingEnabled)
            {
            ...
            do
            {
                ...
                {
                    Log.LogDebug("Operation {0} succeeded {1} for key {2} : {3}", operation.GetType().Name,
                ...
                {
                    Log.LogDebug("Operation doesn't support retries for key {0}", operation.Key);
                    break;
                }
            ...
            CancellationTokenSource cts = null)
        {
            ...
            try
            {
                ...
                Log.LogDebug("Starting send for {0} with {1}", operation.Opaque, server.EndPoint);
                server.SendAsync(operation).ConfigureAwait(false);
            }
            catch (Exception e)
            {
                    ...
                    queryResult = await RetryQueryEveryAsync(async (e, c) =>
                    {
                        ...
                        if (!result.Success)
                        {
                            ...
                            if (exception != null)
                            {
                                lock (_syncObj)
                                {
                                    Log.LogTrace("Request failed checking config:", exception);
                                    UpdateConfig();
                                }
                            }
                        }
