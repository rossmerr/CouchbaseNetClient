...
namespace Couchbase.Core.Buckets
{
    ...
    internal sealed class KeyObserver
    {
            ...
            PersistTo persistTo)
        {
            ...
            using (var cts = new CancellationTokenSource(_timeout))
            {
                ...
                var task = await ObserveEvery(async p =>
                {
                    ...
                    Log.LogDebug("Master {0} - {1}", master.EndPoint, result.Value);
        ...
        async Task<bool> CheckReplicaAsync(ObserveParams observeParams, Observe operation, int replicaIndex)
        {
            Log.LogDebug("checking replica {0}", replicaIndex);
            ...
            Log.LogDebug("Replica {0} - {1} {2}", replica.EndPoint, result.Value.KeyState, replicaIndex);
            var state = result.Value;
            if (state.KeyState == observeParams.Criteria.PersistState)
            {
                Interlocked.Increment(ref observeParams.ReplicatedToCount);
